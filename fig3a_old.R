...
data0 <- data[isAPOBEC==0]
data1 <- data[isAPOBEC==1]
...
dataMerge <- merge(data1,data0,by=c("cancer","structure","sample"))

dataMerge[, ratio := log((cntIn/trgIn)/(cntOut/trgOut))]
...
dataMerge <- merge(dataMerge,activity,by="sample",all.x = TRUE)
...
for(s in structures)
{
  for(c in cancers)
  {
      dt <- dataMerge[cancer==c & structure == s]
      
      p <- ggplot(dt,aes(x=enrichment,y=ratio)) + geom_point(color=rgb(38,120,178,maxColorValue = 255)) +
      ...
      ggsave(paste0("/Users/mar/BIO/PROJECTS/APOBEC/NONBDNA/pics/fig5/a/",s,"_",c,".tiff"),units="mm",width=100,height=60,dpi=300)
