...
for(canc in c("BLCA","BRCA","HNSC","CESC","LUAD","LUSC")){
 dt <- data[cancer == canc]
 dt <- dt[,.(enrichment,apr_ratio,dr_ratio,gq_ratio,ir_ratio,mr_ratio,str_ratio,z_ratio)]
 dtmelt <- melt(dt,id.vars = "enrichment")
 dtmelt <- data.table(dtmelt)
 for(str in c("apr_ratio","dr_ratio","gq_ratio","ir_ratio","mr_ratio","str_ratio","z_ratio")){
  dtt <- dtmelt[variable == str]
  ggplot(dtt, aes(x=enrichment,y=value)) + geom_point()
  ggsave(paste0("/Users/mar/BIO/PROJECTS/APOBEC/NONBDNA/Gordenin/plots_points/",clusterType[[i]],"_point_",canc,"_",str,".tiff"))
 }
}
